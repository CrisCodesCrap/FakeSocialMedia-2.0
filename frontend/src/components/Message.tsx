...
export default function Message({message, isCurrentUser, timestamp, user, key}:MessageProps) {
    ...
    if(isCurrentUser){
        return ( 
            <div key={key} className='inline-flex flex-row-reverse w-full items-center'>
                <div className='rounded-full bg-purple-700 w-9 h-9 mr-3'/>
                <div className='inline-flex flex-col bg-purple-600 m-2 p-1.5 text-ellipsis h-fit w-2/3 text-white rounded-3xl'>
    ...
    return ( 
        <div key={key} className='inline-flex flex-row w-full items-center'>
